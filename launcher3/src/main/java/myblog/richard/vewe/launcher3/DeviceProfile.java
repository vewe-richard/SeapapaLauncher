...
public class DeviceProfile {
    ...
    public void layout(Launcher launcher) {
        ...
        if (overviewMode != null) {
            ...
            int visibleChildCount = 3;  //getVisibleChildCount(overviewMode);
            int totalItemWidth = visibleChildCount * overviewModeBarItemWidthPx;
            int maxWidth = totalItemWidth + (visibleChildCount-1) * overviewModeBarSpacerWidthPx;

            lp.width = Math.min(availableWidthPx, maxWidth);
            lp.height = r.height() * 5 / 2;
            overviewMode.setLayoutParams(lp);

            TextView tv = (TextView)overviewMode.findViewById(R.id.app_types_button);
            if(tv != null){
                Drawable[] dbs = tv.getCompoundDrawables();
                if(dbs[1] != null){
                    dbs[1].setColorFilter(0xff00aa00, PorterDuff.Mode.SRC_ATOP);
                    tv.setCompoundDrawables(null, dbs[1],null, null);
                }
                tv.setText(tv.getText() + UsersContract.TableApplist.FORBIDDEN_PREFIX);
            }

            tv = (TextView)overviewMode.findViewById(R.id.student_setting_button);
            if(tv != null){
                Drawable[] dbs = tv.getCompoundDrawables();
                if(dbs[1] != null){
                    dbs[1].setColorFilter(0xff99FFFF, PorterDuff.Mode.SRC_ATOP);
                    tv.setCompoundDrawables(null, dbs[1], null, null);
                }
                tv.setText(tv.getText() + UsersContract.TableApplist.FORBIDDEN_PREFIX);
            }

            tv = (TextView)overviewMode.findViewById(R.id.parent_setting_button);
            if(tv != null){
                Drawable[] dbs = tv.getCompoundDrawables();
                if(dbs[1] != null){
                    dbs[1].setColorFilter(0xffe6b121, PorterDuff.Mode.SRC_ATOP);
                    tv.setCompoundDrawables(null, dbs[1], null, null);
                }
                tv.setText(tv.getText() + UsersContract.TableApplist.FORBIDDEN_PREFIX);
            }
